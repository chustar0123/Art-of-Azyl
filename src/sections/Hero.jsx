...
const Hero = ({ scrollToSection, sectionRefs }) => {
  ...
  const handleExploreWork = () => {
    if (scrollToSection && sectionRefs && sectionRefs.gallery) {
      scrollToSection(sectionRefs.gallery);
    }
  };
        ...
        <div className="relative z-10 w-full flex flex-col lg:flex-row items-center gap-10 lg:gap-0 px-4 sm:px-8 md:px-12 lg:px-20">
          ...
          <div className="w-full lg:w-1/2 flex justify-center lg:justify-start">
            ...
            >
              ...
              >
                Bringing your ideas to life through custom pencil portraits, murals, canvas paintings, and digital art.
              </p>

              <button 
                onClick={handleExploreWork}
                className="bg-green-500 text-black px-5 py-2.5 sm:px-6 sm:py-3 rounded-full font-semibold hover:bg-green-400 active:scale-95 transition-all text-sm sm:text-base relative z-10 cursor-pointer"
                aria-label="Explore artwork gallery"
              >
                Explore Work
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              {slides.map((slide, i) => (
                <button
                  key={i}
                  onClick={() => goTo(i)}
                  className="relative overflow-hidden rounded-xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-green-500"
                  style={{
